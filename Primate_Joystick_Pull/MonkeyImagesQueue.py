...
class MonkeyImages(tk.Frame,):
    def __init__(self, parent, *args, **kwargs):
        ...
        unpausebutton = tk.Button(self.root, text = "Unpause", height = 5, width = 5, command = self.Unpause)
            ...
            if self.MonkeyLoop == True:
                try:
                    ...
                    if msg == "New Image":
                        # print('New Image')
                        self.next_image()
                    elif msg == "Water Reward":
                        print('water')
                        # Water Reward something?
                        pass
                except:
                    pass
                if self.readyforplexon == True:
                ...
                if self.StartTrialBool == True and self.RelStartTime <=  self.PictureCueTimeInterval:
                    self.StartTrialCue()
...
############################################################################################################################################
    def AdaptiveRewardThreshold(self, AdaptiveValue, AdaptiveAlgorithm):
        ...
        if AdaptiveAlgorithm == 1: # 1: Value Change - Ryan
...
############################################################################################################################################
    ...
    # Currently Not used: EndTrialCue
    ...
    def HighLevelRewardOn(self):
        ...
        print('Image Reward coming soon to a Pedal Press Project near you!')
        # self.counter = -1 ### Monkey Image Number -1 if image is last in directory
        self.ImageReward = True

    def HighLevelRewardOff(self):
        print('Image Reward Off')
        print('Image Reward coming soon to a Pedal Press Project near you!')
        # self.counter = 0 # No image
        self.ImageReward = False
