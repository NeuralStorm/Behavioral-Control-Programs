    ...
    def LOOP(self): #LOOP will be different for each experiment
        #try: #For Later when we want a try block to catch exceptions.
            if self.MonkeyLoop == True:
                ...
                if self.StartTrialBool == True and self.PunishLockout == False and self.Area1_right_pres == False and self.Area1_left_pres == False:
...
################################################################################################################################################################################################
                    ...
                    self.counter = 1 # TRAINING: Only use first DS
                ...
                if self.TrainingStart == True and self.RelCueTime >= self.TrainingDuration and self.RewardOccurred == False: # TODO: Proper End of task reward requirements for after DS, keep hand in place.
                    print('water reward')
