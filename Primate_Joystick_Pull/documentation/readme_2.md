...
Duartion of pull is within 4 seconds (0-4), the reward is linearly related between 0 and 1 when the oval is cued
...
A number in the range [`Pre Go Cue Min delta t2`, `Pre Go Cue Max delta t2`] will be selected each trial for the delay between a shape being shown and the go cue being shown. Min and max can be set to the same val`gue to have a fixed delay.
