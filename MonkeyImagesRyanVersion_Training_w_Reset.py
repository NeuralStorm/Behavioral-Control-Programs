...
# TRAINING NOTES:
# self.TrainingDuration is the duration for the monkey to have his hand in Area1 for a reward.
...
class MonkeyImages(tk.Frame,):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.RewardTime = 0.25
        self.ResetTime = 3                                  # This is the Time between Reward and beginning of next trial
        ...
        self.RewardOccurredTime = time.time()
        self.RelRewardOccurredTime = time.time() - self.RewardOccurredTime
    ...
    def LOOP(self): #LOOP will be different for each experiment
        #try: #For Later when we want a try block to catch exceptions.
            if self.MonkeyLoop == True:
                ...
                if self.StartTrialBool == True and self.PunishLockout == False and self.Area1_right_pres == False:
                    print('waiting for home area sound')
                    self.StartTrialCue()
                
                if self.Area1_right_pres == True and self.TrainingStart == False:
                ...
                if self.TrainingStart == True and self.RelTrainingStartTime >= self.TrainingDuration and self.RewardOccurred == False:
                    print('water reward')
                ...
                if self.RewardOccurred == True and self.RelRewardOccurredTime >= self.ResetTime:
                    self.RewardOccurred = False
                ...
                if self.TrainingStart == True:
                    self.RelTrainingStartTime = time.time() - self.TrainingStartTime
                if self.Area1_right_pres == False:
                    self.StartTrialBool = True
                    self.RewardOccurred = False
                    self.TrainingStart = False
                self.RelRewardOccurredTime = time.time() - self.RewardOccurredTime
